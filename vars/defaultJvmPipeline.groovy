...
void call(final Map<String, ? extends Serializable> config) {
  try {
    ...
    final Set<String> tests = (Set<String>) config.getOrDefault('tests', [].toSet())
    final boolean gradlePlugin = config.getOrDefault('gradlePlugin', Boolean.FALSE)

    final String projectName = ((String)JOB_NAME).split('/')[0]

    final String branchName = env.BRANCH_NAME
    final boolean isMaster = branchName == 'master'
    final boolean isNotPR = !env.CHANGE_ID
    final boolean isMasterAndNotPR = isMaster && isNotPR
    ...
    node { ->
      ansiColor { ->
        stage('Checkout') { ->
          final List<Map<String, ? extends Serializable>> extensions = [
            [$class: 'WipeWorkspace'],
        ...
        final GradleBuild rtGradle = Artifactory.newGradleBuild()
        rtGradle.useWrapper = true
        rtGradle.usesPlugin = true

        final boolean alwaysLinkReportsToLastBuild = isMasterAndNotPR
         ...
         * <grv87 2018-09-22>
         */
        final String gradleSwitches = '-Dorg.gradle.internal.launcher.welcomeMessageEnabled=false --no-daemon --parallel --configure-on-demand --console=plain --info --warning-mode all --full-stacktrace --build-cache'
        ...
        withGpg(artifactoryServerId, gpgKeyCredentialId, gpgPassphraseCredentialId, true) { String fingerprint ->
          withEnv([
            ...
            List credentials = [
              ...
              string(credentialsId: githubCredentialId, variable: 'ORG_GRADLE_PROJECT_ghToken'),
              usernamePassword(credentialsId: 'Artifactory', usernameVariable: 'ORG_GRADLE_PROJECT_artifactoryUser', passwordVariable: 'ORG_GRADLE_PROJECT_artifactoryPassword'), // TODO
              string(credentialsId: gpgPassphraseCredentialId, variable: 'ORG_GRADLE_PROJECT_gpgKeyPassphrase'),
            ]
            if (publicReleases) {
            ...
            withCredentials(credentials) { ->
              ...
              final List<AnnotatedReport> issues = []
              try {
                stage('Generate Changelog') { ->
                  timeout(time: timeouts.getOrDefault('Generate Changelog', 5), unit: 'MINUTES') { ->
                    ...
                    EnvExcludes.EXCLUDES.each { String exclude ->
                      buildInfo.env.filter.addExclude exclude
                    }
